    ...
    def _build_results_scenario(
        ...
        *,
        ...
        helper_script = (
            ...
            "f.a=()=>{let o=f.x(),r=f.o(),t=o[__FACET_INDEX__]||o[0];if(!t)return 0;f.g={s:t.n,o:o.map(v=>({n:v.n,p:v.p}))};for(let e of r?r.querySelectorAll(b):[])if(/clear all/i.test(e.innerText)){e.click();break}setTimeout(()=>{let x=f.x().find(v=>v.n==t.n);x&&x.e.click()},500);return 1};"
            "f.c=()=>{for(let e of document.querySelectorAll(b+',[role=\"tab\"]'))if(f.v(e)&&/^cheapest$/i.test(f.t(e.innerText))){e.click();return 1}return 0};"
