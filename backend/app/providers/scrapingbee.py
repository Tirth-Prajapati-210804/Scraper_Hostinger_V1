...
class ScrapingBeeProvider:
    ...
    def _build_multi_city_results_scenario(self, *, deep: bool = False) -> dict[str, object]:
        ...
        helper_script = (
            ...
            "fh.timeRe=/\\b\\d{1,2}:\\d{2}\\s*[ap]m\\b/gi;"
            ...
            "};"
            "fh.matchCount=(text,re)=>{const matches=fh.clean(text).match(re);re.lastIndex=0;return matches?matches.length:0;};"
            "fh.extractPriceText=card=>{"
            "const direct=fh.clean(card.querySelector('.nrc6-price-section .e2GB-price-text,[class*=\"price-text\"],[data-testid*=\"price\"]')?.innerText);"
            "if(direct)return direct;"
            "const text=fh.clean(card.innerText);"
            "const match=text.match(fh.priceRe);"
            "return fh.clean(match&&match[0]);"
            "};"
            "fh.legNodes=card=>{"
            "const primary=Array.from(card.querySelectorAll('ol.hJSA-list > li')).filter(li=>fh.visible(li)&&fh.clean(li.innerText));"
            "if(primary.length>=2)return primary.slice(0,2);"
            "const fallback=Array.from(card.querySelectorAll('ol li,[role=\"listitem\"],li')).filter(li=>fh.visible(li)&&fh.clean(li.innerText));"
            "return fallback.slice(0,2);"
            "};"
            "fh.findCardRoot=node=>{"
            "let current=node,fallback=null;"
            "for(let depth=0;current&&depth<10;depth+=1,current=current.parentElement){"
            "if(!fh.visible(current))continue;"
            "const text=fh.clean(current.innerText);"
            "if(!text)continue;"
            "const timeHits=fh.matchCount(text,fh.timeRe);"
            "const hasStops=/\\b(?:direct|\\d+\\s+stop(?:s)?)\\b/i.test(text);"
            "const hasPrice=fh.priceRe.test(text);"
            "const hasSelect=/\\bselect\\b/i.test(text);"
            "if(hasPrice&&(timeHits>=4||hasStops))fallback=current;"
            "if(hasPrice&&timeHits>=4&&hasSelect)return current;"
            "}"
            "return fallback;};"
            f"fh.captureCards=()=>{{const cardLimit={card_limit};"
            "const isCard=node=>{"
            "if(!node||!fh.visible(node))return false;"
            "const text=fh.clean(node.innerText);"
            "if(!text)return false;"
            "const hasPrice=!!fh.extractPriceText(node);"
            "const hasLegs=fh.legNodes(node).length>=2||fh.matchCount(text,fh.timeRe)>=4;"
            "return hasPrice&&hasLegs;};"
            ...
            "price_text:fh.extractPriceText(card),"
