...
class ScrapingBeeProvider:
    ...
    def _build_multi_city_results_scenario(self, *, deep: bool = False) -> dict[str, object]:
        ...
        helper_script = (
            ...
            "if(node.querySelector('.nrc6-price-section .e2GB-price-text')&&node.querySelectorAll('ol.hJSA-list > li').length>=2)return node;"
            "}"
            "return null;};"
            f"fh.captureCards=()=>{{const cardLimit={card_limit};"
            "const isCard=node=>!!node&&fh.visible(node)&&!!node.querySelector('.nrc6-price-section .e2GB-price-text')&&node.querySelectorAll('ol.hJSA-list > li').length>=2;"
            "const selectorMatches=Array.from(document.querySelectorAll('div[aria-label^=\"Result item\"],div[data-resultid],div.nrc6,div[class*=\"nrc6\"]')).filter(isCard);"
